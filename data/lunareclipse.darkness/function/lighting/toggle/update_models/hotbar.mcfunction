...
$execute if items entity @s hotbar.$(index) minecraft:$(item)[minecraft:custom_data={$(tag):1b}] run item modify entity @s hotbar.$(index) { "function": "minecraft:set_components", "components": { "minecraft:item_model": "$(model)" } }